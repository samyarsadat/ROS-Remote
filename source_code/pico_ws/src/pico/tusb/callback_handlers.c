...
void tud_hid_set_report_cb(uint8_t instance, uint8_t report_id, hid_report_type_t report_type, uint8_t const* buffer, uint16_t bufsize) {
    ...
    if (report_type == HID_REPORT_TYPE_OUTPUT) {
        // No need to check instance, as report IDs are unique across instances.

        switch (report_id) {
            case LED_OUTPUT_REPORT_ID:
                if (bufsize == sizeof(hid_led_report_t)) {
                ...
                break;
            case INPUT_POLL_OUTPUT_REPORT_ID:
...
// Only thing the host can request is the LED states feature report.
uint16_t tud_hid_get_report_cb(uint8_t instance, uint8_t report_id, hid_report_type_t report_type, uint8_t* buffer, uint16_t reqlen) {
    (void) instance;

    if (report_type == HID_REPORT_TYPE_FEATURE && 
        report_id == LED_STATES_FEATURE_REPORT_ID) 
    {
        const uint16_t struct_size = sizeof(hid_led_states_report_t);

        if (reqlen < struct_size) {
            LOG(LOG_LVL_ERROR, "LED states feature request length too short!");
            return 0;
        }

        hid_led_states_report_t* report = (hid_led_states_report_t*) buffer;
        const led_state_t* states = get_led_states();
        for (int i = 0; i < NUMBER_OF_LEDS; i++) {
            report->mode[i] = states[i].mode;
            report->pwm_out[i] = states[i].pwm_set_out;
        }
    ...
    if (report_type == HID_REPORT_TYPE_INPUT) {
        ...
        switch (report_id) {
            ...
            case POT_INPUT_REPORT_ID:
                ...
                break;
            default:
                LOG(LOG_LVL_DEBUG, "Unhandled report ID for failed HID report.");
                break;
        }
    }
}
